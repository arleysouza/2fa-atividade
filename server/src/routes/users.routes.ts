...
import { changePassword, createUser, loginUser, logoutUser, verifyMfaCode } from "../controllers/user.controller";
...
router.post(
  ...
  validateBody([
    ...
    { name: "password", required: true, type: "string", minLength: 8, pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).+$/ },
...
router.patch(
  ...
  validateBody([
    ...
    { name: "newPassword", required: true, type: "string", minLength: 8, pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).+$/ },
  ]),
  changePassword,
);
